...
describe("Change the nullGetter", function () {
	it("should work with null", function () {
		...
		var xmlTemplater = createXmlTemplaterDocx(content, { tags: scope, nullGetter: nullGetter });
		xmlTemplater.render();
...
describe("Custom delimiters", function () {
	it("should work with custom tags", function () {
		...
		var content = "<w:t>Hello [name]</w:t>";
		var scope = { name: "Edgar" };
		var xmlTemplater = createXmlTemplaterDocx(content, { tags: scope, delimiters: delimiters });
		xmlTemplater.render();
		expect(xmlTemplater.getFullText()).to.be.equal("Hello Edgar");
	});
	...
	it("should work with loops", function () {
		...
		var xmlt = createXmlTemplaterDocx(content, { tags: { loop: [{ innertag: 10 }, { innertag: 5 }] } }).render();
	...
	it("should work with complex loops (1)", function () {
		...
		var xmlt = createXmlTemplaterDocx(content, { tags: { looptag: true, innertag: "foo" } }).render();
	...
	it("should work with complex loops (2)", function () {
		...
		var xmlt = createXmlTemplaterDocx(content, { tags: { person: [{ name: "Henry" }] } }).render();
...
describe("getting parents context", function () {
	it("should work with simple loops", function () {
		...
		var xmlt = createXmlTemplaterDocx(content, { tags: { loop: [1], name: "Henry" } }).render();
	...
	it("should work with double loops", function () {
		...
		var xmlt = createXmlTemplaterDocx(content, { tags: { loop_first: [1], loop_second: [{ name_inner: "John" }], name_outer: "Henry" } }).render();
